...
# This is table for Users information gotten from registration
...
#This is the table for storing Errands Requests Informations
...
# This is the table that sored the chatlist of a user
class Chat(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    chat_list = db.Column(db.JSON, nullable=False, default=list)
    user = db.relationship('User', foreign_keys=[user_id], backref=db.backref('chats', lazy=True))

    def save_to_db(self):
        db.session.add(self)
        db.session.commit()

# This is used to store the messages sent and received by users
